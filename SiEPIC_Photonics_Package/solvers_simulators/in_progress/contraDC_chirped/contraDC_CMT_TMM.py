...
def contraDC_model(contraDC, simulation_setup, waveguides,plot = True):
    ...
    N_seg = int(1e9*abs(contraDC.period1-contraDC.period2))                 # Number of flat steps along the device
    ...
    for ii in range(lenghthLambda):
        ...
        for n in range(N_seg):
            contraDC_period1 = contraDC.period1
            contraDC_period2 = contraDC.period2

            linear_chirp = n*(contraDC_period2-contraDC_period1)/N_seg+contraDC_period1
            period = linear_chirp
            
            if n == 0 or n == N_seg-1:
                l_seg = (contraDC.N / (2*N_seg)) *period
            else:
                l_seg = (contraDC.N / (N_seg)) *period
