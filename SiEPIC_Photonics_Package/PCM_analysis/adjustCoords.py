...
@description: Load SiEPIC-Tools coordinates files and adjust them for automated measurements
"""
...
def adjustCoords( f_name, del_comment, num_fields):    
    ...
    num_fields : int
        Number of acceptable fields in the format, i.e., if the format is (X, Y, pol, wavelength, type, deviceID) then num_fields = 6
    ...
    for line in f:
        ...
        # reduce an N numbered list to a list of size num_fields, concatenate extra elements with underscore '_'
...
del_comment = True # remove additional ", comment" markers
num_fields = 6 # number of acceptable file fields (X, Y, pol, wavelength, type, deviceID), any fields after the Nth field will be appended to the last field by an underscore
